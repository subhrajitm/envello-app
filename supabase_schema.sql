...
-- 12. Journal Projects
-- ──────────────────────────────────────────────────────
create table if not exists public.journal_projects (
  id text primary key,
  user_id uuid references auth.users(id) not null default auth.uid(),
  title text,
  description text,
  entries_count double precision,
  active boolean,
  word_count double precision,
  target_word_count double precision,
  progress double precision,
...
alter table public.journal_projects enable row level security;
create policy "Users can all access their own journal projects" on public.journal_projects for all using (auth.uid() = user_id);
...
create table if not exists public.journal_entries (
  ...
  preview text,
  type text,
  column_id text, -- renamed from column to avoid keyword issues if any
  tags jsonb,
  word_count double precision,
  character_count double precision,
  created_date text,
  last_edited text,
  has_ai boolean,
...
create policy "Users can all access their own journal entries" on public.journal_entries for all using (auth.uid() = user_id);
...
create policy "Users can all access their own journal columns" on public.journal_columns for all using (auth.uid() = user_id);
