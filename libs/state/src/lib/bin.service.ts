...
export class BinService {
  ...
  addToBin(item: Omit<BinItem, 'id' | 'deletedAt'>) {
    ...
    this.db
      .upsert('bin_items', binItem)
      .catch((e) => console.error('[BinService] persist bin item failed', e));
  }

  permanentlyDelete(binItemId: string) {
    this.items.update((list) => list.filter((i) => i.id !== binItemId));
    this.db
      .remove('bin_items', binItemId)
      .catch((e) => console.error('[BinService] remove bin item failed', e));
  }
