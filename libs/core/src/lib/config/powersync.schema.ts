...
const habits = new Table(
  {
    ...
    targetDays:  column.text,  // JSON number[]
    color:       column.text,
    icon:        column.text,
    logs:        column.text,  // JSON HabitLog[]
    createdAt:   column.text,
  ...
  {
    localOnly: true,
    indexes: { by_profile: ['profile_id'], by_profile_archived: ['profile_id', 'archivedAt'] },
  }
);
