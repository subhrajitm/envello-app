...
describe('AuthService', () => {
    ...
    beforeEach(() => {
        const supabaseSpy = jasmine.createSpyObj('SupabaseService', ['getSession', 'authChanges', 'signIn', 'signOut', 'signUp']);
        // Setup default returns
        supabaseSpy.getSession.and.returnValue(Promise.resolve({ data: { session: null }, error: null }));
        ...
        const loggingMock = jasmine.createSpyObj('LoggingService', ['info', 'error']);
    ...
    describe('Initialization', () => {
        ...
        it('should set authenticated state if session exists', async () => {
            ...
            supabaseServiceSpy.getSession.and.returnValue(Promise.resolve({ data: { session: mockSession }, error: null }));
    ...
    describe('Login', () => {
        it('should call supabase signInWithPassword and return true on success', async () => {
            ...
            (authClient.signInWithPassword as jasmine.Spy).and.returnValue(Promise.resolve({ data: {}, error: null }));

            const result = await service.login('test@test.com', 'pass');

            expect(authClient.signInWithPassword).toHaveBeenCalledWith({ email: 'test@test.com', password: 'pass' });
            expect(result).toBeTrue();
        });

        it('should return false on error', async () => {
            const authClient = supabaseServiceSpy.client.auth;
            (authClient.signInWithPassword as jasmine.Spy).and.returnValue(Promise.resolve({ data: null, error: { message: 'Failed' } }));
