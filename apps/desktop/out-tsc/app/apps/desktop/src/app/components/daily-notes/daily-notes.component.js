...
import { Component, computed, inject, signal, untracked, HostListener, effect } from '@angular/core';
...
let DailyNotesComponent = class DailyNotesComponent {
    ...
    // Modal State
    activeModal = signal('none');
    modalInputValue = signal('');
    modalInputPlaceholder = signal('');
    modalTitle = signal('');
    tempNoteId = signal(''); // For storing ID during confirmation flow
    tempFolderId = signal(''); // For storing Folder ID during confirmation flow
    selectedEntryId = signal('');
    searchQuery = signal('');
    selectedFilter = signal('all'); // all, pinned, tagged, archived
    activeView = signal('folders'); // Track active sidebar view
    ...
    dragOverFolderId = signal(null);
    draggingNoteId = signal(null);
    SELECTION_KEY = 'envello-daily-notes-selection';
    ...
    filteredNotes = computed(() => {
        ...
        // Apply search filter
        if (query) {
            list = list.filter(note => note.title.toLowerCase().includes(query) ||
    ...
    getBucketedNotesForFolder(folderId) {
    ...
    constructor() {
        ...
        effect(() => {
            ...
            if (hasValidSelection && hasValidTabs)
                return;
        ...
        effect(() => {
            const selectedId = this.selectedEntryId();
            const openIds = this.openNotes();
            if (!selectedId && openIds.length === 0)
                return;
    ...
    updateNoteContent(content, preview) {
    ...
    onFolderDragLeave(event) {
        ...
        if (related?.closest?.('.folder-group, .dn-entries-list'))
            return;
        this.dragOverFolderId.set(null);
    }
    ...
    onListDragOver(event) {
        if (event.target?.closest?.('.folder-group'))
            return;
        event.preventDefault();
        if (event.dataTransfer)
            event.dataTransfer.dropEffect = 'move';
    ...
    onListDragLeave(event) {
        if (event.relatedTarget?.closest?.('.dn-entries-list'))
            return;
        this.dragOverFolderId.set(null);
    }
    onListDrop(event) {
        ...
        const firstId = this.noteGroups()[0]?.id ?? 'personal';
        if (noteId)
            this.moveNoteToFolder(noteId, firstId);
    }
    ...
    saveSelectionToStorage(selectedId, openNotes) {
        if (typeof localStorage === 'undefined')
            return;
        try {
    ...
    toggleGroup(groupId) {
        this.noteGroups.update(groups => groups.map(g => g.id === groupId ? { ...g, expanded: !g.expanded } : g));
    }
    setFilter(filter) {
    ...
    toggleTagCategory(categoryId) {
        this.tagCategories.update(categories => categories.map(c => c.id === categoryId ? { ...c, expanded: !c.expanded } : c));
    }
    ...
    toggleExpandAllTags() {
        ...
        this.tagCategories.update(categories => categories.map(c => ({ ...c, expanded: shouldExpand })));
    }
    ...
    requestDeleteNote(noteId, event) {
        event.stopPropagation();
...
], DailyNotesComponent.prototype, "handleEscape", null);
__decorate([
    HostListener('document:click', ['$event'])
], DailyNotesComponent.prototype, "onDocumentClick", null);
DailyNotesComponent = __decorate([
    Component({
