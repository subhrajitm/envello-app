...
import { Component, signal, effect, inject, computed, HostListener, ViewChild, ChangeDetectionStrategy } from '@angular/core';
...
let NovelEditorComponent = class NovelEditorComponent {
    ...
    confirmAdd() {
        ...
        if (modal.type === 'act') {
            ...
            this.novelService.addNote(modal.inputValue.trim(), modal.inputValue2?.trim() || '', this.activeChapterId() || undefined);
        }
        this.cancelAdd();
    }
    cancelAdd() {
    ...
    totalNovelWords = computed(() => {
        ...
        if (novel.prologue)
            total += novel.prologue.wordCount;
    ...
    filteredChapters = computed(() => {
        ...
        if (!query || query.length < 2)
            return null; // Early return for short queries
        const novel = this.novel();
        if (!novel)
            return null;
        const results = [];
    ...
    applySuggestion(suggestion) {
        ...
        // Remove suggestion
        this.aiSuggestions.update(suggestions => suggestions.filter(s => s.id !== suggestion.id));
    }
    clearAiConversation() {
        this.aiMessages.set([]);
    ...
    insertLink() {
        if (!this.editor || !this.linkUrl().trim())
            return;
...
__decorate([
    ViewChild('addInput')
], NovelEditorComponent.prototype, "addInputRef", void 0);
__decorate([
    HostListener('document:click', ['$event'])
], NovelEditorComponent.prototype, "onDocumentClick", null);
__decorate([
    HostListener('document:keydown', ['$event'])
], NovelEditorComponent.prototype, "onKeyDown", null);
