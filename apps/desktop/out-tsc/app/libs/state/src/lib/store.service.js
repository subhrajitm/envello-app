...
let StoreService = class StoreService {
        ...
        try {
            ...
            if (folders?.length) {
                ...
                for (const f of defaultFolders) {
                    this.db.upsert('note_folders', f).catch((e) => console.error('[StoreService] persist note_folders failed', e));
                }
            }
        }
        catch (e) {
    ...
    updateNote(id, updates) {
        ...
        this.notes.update(notes => notes.map(note => note.id === id ? {
            ...note,
            ...updates,
    ...
    updateProject(id, updates) {
        this.projects.update(projects => projects.map(p => p.id === id ? { ...p, ...updates } : p));
    ...
    addNoteFolder(folder) {
        ...
        this.db.upsert('note_folders', folder).catch(e => console.error('[StoreService] persist note_folders failed', e));
    }
    removeNoteFolder(id) {
        this.noteFolders.update(list => list.filter(f => f.id !== id));
        this.db.remove('note_folders', id).catch(e => console.error('[StoreService] remove note_folder failed', e));
    }
};
