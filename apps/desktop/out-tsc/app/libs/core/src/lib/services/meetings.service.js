...
let MeetingsService = class MeetingsService {
    ...
    filteredMeetings = computed(() => {
        ...
        if (labels.length > 0) {
            result = result.filter(m => labels.some(label => m.labels?.includes(label)));
        }
        // Apply search
    ...
    updateMeeting(id, updates) {
        this.meetings.update(meetings => meetings.map(meeting => meeting.id === id
    ...
    updateAgendaItem(meetingId, itemId, updates) {
        ...
        if (!meeting?.agenda)
            return;
        this.updateMeeting(meetingId, {
            agenda: meeting.agenda.map(item => item.id === itemId ? { ...item, ...updates } : item),
        });
    }
    ...
    updateActionItem(meetingId, itemId, updates) {
        ...
        if (!meeting?.actionItems)
            return;
        this.updateMeeting(meetingId, {
            actionItems: meeting.actionItems.map(item => item.id === itemId ? { ...item, ...updates } : item),
        });
    }
    ...
    updateNote(meetingId, noteId, content) {
        ...
        if (!meeting?.notes)
            return;
        this.updateMeeting(meetingId, {
            notes: meeting.notes.map(note => note.id === noteId ? { ...note, content } : note),
        });
    }
    ...
    updateAttendee(meetingId, attendeeId, updates) {
        ...
        if (!meeting)
            return;
        this.updateMeeting(meetingId, {
            attendees: meeting.attendees.map(a => a.id === attendeeId ? { ...a, ...updates } : a),
        });
    }
    ...
    getRelativeDateLabel(meeting) {
        if (this.isMeetingToday(meeting))
            return 'Today';
        if (this.isMeetingTomorrow(meeting))
            return 'Tomorrow';
        const date = new Date(meeting.date);
        const today = new Date();
