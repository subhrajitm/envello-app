...
let ResearchComponent = class ResearchComponent {
    ...
    // UI State
    viewMode = signal('libraries');
    selectedLibrary = signal(null);
    // Modals
    ...
    libraryToDelete = signal(null);
    sourceToDelete = signal(null);
    // Filter & Search
    searchQuery = signal('');
    ...
    filteredLibraries = computed(() => {
        ...
        if (!q)
            return list;
        return list.filter(lib => lib.name.toLowerCase().includes(q) ||
    ...
    formatSourceMeta(source) {
        const parts = [this.getSourceTypeLabel(source.sourceType)];
        if (source.author)
            parts.push(source.author);
        const date = source.lastAccessed || source.createdDate;
        if (date)
            parts.push(new Date(date).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }));
        return parts.join(' · ');
    }
    // AI Features
    toggleAIPanel() {
