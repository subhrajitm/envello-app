...
let CodeSnippetsComponent = class CodeSnippetsComponent {
    ...
    addSnippet() {
        ...
        if (tagStr) {
            tagStr.split(/[\s,]+/).forEach(t => { const x = t.replace(/^#/, '').trim(); if (x && !tags.includes(x))
                tags.push(x); });
        }
...
], CodeSnippetsComponent.prototype, "onDocumentClick", null);
__decorate([
    HostListener('document:keydown', ['$event'])
], CodeSnippetsComponent.prototype, "onKeyDown", null);
