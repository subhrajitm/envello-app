...
let MeetingsComponent = class MeetingsComponent {
    ...
    recurringPattern = signal('weekly');
    recurringCount = signal(4);
    // Meeting details modal
    showDetailsModal = signal(false);
    selectedMeeting = signal(null);
    editingMeeting = signal(false);
    editedMeeting = signal({});
    // Details tabs
    detailsTab = signal('overview');
    // Action items
    newActionItemInput = signal('');
    newActionItemAssignee = signal('');
    newActionItemPriority = signal('MEDIUM');
    // Notes
    newNoteInput = signal('');
    // Agenda editing
    ...
    filteredMeetings = computed(() => {
        ...
        if (proj) {
            meetings = meetings.filter(m => proj === 'No project' ? !m.project : m.project === proj);
        }
    ...
    getMeetingsForDate(date) {
        ...
        return this.meetingsService.meetings().filter(m => m.date === dateStr && m.status !== 'cancelled');
    }
    // Get unique projects
    ...
    stats = computed(() => {
        ...
        const openActionItems = all.reduce((sum, m) => sum + (m.actionItems?.filter(a => a.status !== 'completed').length ?? 0), 0);
    ...
    handleKeyboardShortcuts(event) {
        ...
        if (event.key === '2')
            this.viewFilter.set('today');
        if (event.key === '3')
            this.viewFilter.set('upcoming');
        if (event.key === '4')
            this.viewFilter.set('past');
        // V: Cycle view modes
        if (event.key === 'v') {
            const modes = ['list', 'calendar', 'kanban'];
    ...
    createMeeting() {
        ...
        if (this.showRecurringOptions()) {
            ...
            this.meetingsService.createRecurringSeries(baseMeeting, this.recurringCount());
        }
        else {
    ...
    saveEditing() {
        ...
        if (meeting && edited) {
            ...
            this.selectedMeeting.set({ ...meeting, ...edited });
            this.editingMeeting.set(false);
        }
    }
    cancelEditing() {
    ...
    removeAttendeeFromNew(attendeeId) {
        const current = this.newMeeting();
        this.newMeeting.set({
            ...current,
    ...
    addAttendeeToMeeting() {
        const meeting = this.selectedMeeting();
        if (!meeting || !this.newAttendeeName().trim())
            return;
        ...
        if (updated)
            this.selectedMeeting.set(updated);
    ...
    removeAgendaItemFromNew(itemId) {
        const current = this.newMeeting();
        this.newMeeting.set({
            ...current,
    ...
    addAgendaItem() {
        const meeting = this.selectedMeeting();
        if (!meeting || !this.newAgendaItemTitle().trim())
            return;
    ...
    addActionItem() {
        const meeting = this.selectedMeeting();
        if (!meeting || !this.newActionItemInput().trim())
            return;
    ...
    addNote() {
        const meeting = this.selectedMeeting();
        if (!meeting || !this.newNoteInput().trim())
            return;
    ...
    getCompletedActionItemsCount(meeting) {
        return meeting.actionItems?.filter(a => a.status === 'completed').length ?? 0;
    }
    getActionItemsProgress(meeting) {
    ...
    // Quick actions
    toggleQuickActions(meetingId, event) {
    ...
    updateNewMeetingField(field, value) {
        const current = this.newMeeting();
        this.newMeeting.set({ ...current, [field]: value });
    }
    // Update edited meeting field
    updateEditedMeetingField(field, value) {
        const current = this.editedMeeting();
        this.editedMeeting.set({ ...current, [field]: value });
    }
    // Track by functions
    trackByMeetingId(index, meeting) {
        return meeting.id;
    }
    trackByAttendeeId(index, attendee) {
        return attendee.id;
    }
    trackByAgendaId(index, item) {
        return item.id;
    }
    trackByActionId(index, item) {
        return item.id;
    }
...
], MeetingsComponent.prototype, "onDocumentClick", null);
__decorate([
    HostListener('document:keydown', ['$event'])
], MeetingsComponent.prototype, "handleKeyboardShortcuts", null);
