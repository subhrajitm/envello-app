...
let ArticlesComponent = class ArticlesComponent {
    ...
    selectedPipeline = signal('All Statuses');
    searchQuery = signal('');
    activeArticleId = signal(null);
    showArticleModal = signal(false);
    showDeleteModal = signal(false);
    editingArticle = signal(null);
    articleToDelete = signal(null);
    // Form inputs
    newArticleTitle = signal('');
    newArticlePlatform = signal('Medium');
    newArticlePipeline = signal('DRAFT');
    ...
    platforms = ['All Platforms', 'Medium', 'Substack', 'Blog', 'Dev.to', 'Hashnode', 'Custom'];
    pipelines = ['All Statuses', 'PUBLISHED', 'DRAFT', 'REVIEW', 'SCHEDULED'];
    // Methods
