...
import { Component, signal, HostListener, inject, computed, ViewChild } from '@angular/core';
...
let AddNewModalComponent = class AddNewModalComponent {
    ...
    filteredOptions = computed(() => {
        ...
        if (!query)
            return this.options;
        return this.options.filter(opt => opt.title.toLowerCase().includes(query) ||
    ...
    rightPaneOptions = computed(() => {
        ...
        if (selected === 'all') {
            const recentOptions = recentIds.map(id => this.options.find(o => o.id === id)).filter((o) => !!o);
    ...
    createNovel() {
        ...
        this.novelContentService.createAndPersistEmptyNovel(id, newNovel.title).catch(err => console.error('[AddNewModal] Failed to persist novel content:', err));
...
], AddNewModalComponent.prototype, "searchInput", void 0);
__decorate([
    ViewChild('optionsContainer')
], AddNewModalComponent.prototype, "optionsContainer", void 0);
__decorate([
    HostListener('document:keydown', ['$event'])
], AddNewModalComponent.prototype, "handleKeydown", null);
