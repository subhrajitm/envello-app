...
let SqliteService = class SqliteService {
  ...
  async upsertTask(task) {
    const db = await this.getDb();
    // Check if exists
    const exists = await db.select('SELECT id FROM tasks WHERE id = $1', [
      task.id,
    ]);
    const jsonTask = {
      ...task,
  ...
  async upsertNote(note) {
    const db = await this.getDb();
    const exists = await db.select('SELECT id FROM notes WHERE id = $1', [
      note.id,
    ]);
  ...
  async upsertActivity(act) {
    const db = await this.getDb();
    const exists = await db.select('SELECT id FROM activities WHERE id = $1', [
      act.id,
    ]);
    if (exists.length > 0) {
      await db.execute(
  ...
  async upsertNovel(novel) {
    const db = await this.getDb();
    const exists = await db.select('SELECT id FROM novels WHERE id = $1', [
      novel.id,
    ]);
  ...
  async getNovelContent(id) {
    const db = await this.getDb();
    const rows = await db.select('SELECT * FROM novel_content WHERE id = $1', [
      id,
    ]);
  ...
  async upsertBinItem(item) {
    const db = await this.getDb();
    const exists = await db.select('SELECT id FROM bin_items WHERE id = $1', [
      item.id,
    ]);
  ...
  async upsertSnippet(doc) {
    const db = await this.getDb();
    const exists = await db.select('SELECT id FROM snippets WHERE id = $1', [
      doc.id,
    ]);
  ...
  async upsertBook(doc) {
    const db = await this.getDb();
    const exists = await db.select('SELECT id FROM books WHERE id = $1', [
      doc.id,
    ]);
  ...
  async upsertMeeting(doc) {
    const db = await this.getDb();
    const exists = await db.select('SELECT id FROM meetings WHERE id = $1', [
      doc.id,
    ]);
    const jsonDoc = {
      ...doc,
  ...
  async upsertArticle(article) {
    const db = await this.getDb();
    const exists = await db.select('SELECT id FROM articles WHERE id = $1', [
      article.id,
    ]);
