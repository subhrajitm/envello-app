...
let JournalService = class JournalService {
  ...
  async loadFromDb() {
    try {
      const [projects, entries, cols] = await Promise.all([
        this.db.getAll('journal_projects'),
        this.db.getAll('journal_entries'),
        this.db.getAll('journal_columns'),
      ]);
      this.projects.set(projects);
      this.entries.set(entries);
      if (cols.length === 0) {
        await Promise.all(
          defaultColumns.map((c) => this.db.upsert('journal_columns', c)),
        );
  ...
  async loadEntryContent(id) {
    ...
    if (mdContent) {
      ...
      this.entries.update((es) =>
        es.map((e) => (e.id === id ? { ...e, content: html } : e)),
      );
      return html;
    }
    return '';
  ...
  persistProject(p) {
    this.db
      .upsert('journal_projects', p)
      .catch((e) => logIfTauri('[JournalService] persist project failed', e));
  }
  persistEntry(e) {
    this.db
      .upsert('journal_entries', e)
      .catch((err) => logIfTauri('[JournalService] persist entry failed', err));
  }
  persistColumn(c) {
    this.db
      .upsert('journal_columns', c)
      .catch((e) => logIfTauri('[JournalService] persist column failed', e));
  }
  // Project Management
  getProjects() {
  ...
  setActiveProject(id) {
    ...
    this.store.addActivity(
      `Switched to project: ${this.getProject(id)?.title}`,
      'system',
    );
  ...
  addProject(project) {
    ...
    this.store.addActivity(
      `Journal project created: ${newProject.title}`,
      'system',
    );
  ...
  deleteProject(id) {
    ...
    if (project) {
      ...
      this.store.addActivity(
        `Journal project deleted: ${project.title}`,
        'system',
      );
      this.db
        .remove('journal_projects', id)
        .catch((e) => logIfTauri('[JournalService] remove project failed', e));
    }
  }
  // Entry Management
  ...
  getEntriesByColumn(projectId, columnId) {
    return this.entries().filter(
      (e) => e.projectId === projectId && e.column === columnId,
    );
  }
  getEntry(id) {
  ...
  updateEntry(id, updates) {
    ...
    if (updates.content) {
      ...
      finalUpdates.wordCount = plainText
        .split(/\s+/)
        .filter((w) => w.length > 0).length;
      finalUpdates.characterCount = plainText.length;
      finalUpdates.preview =
        plainText.substring(0, 100) + (plainText.length > 100 ? '...' : '');
      // Debounce File Write
      ...
      entries.map((e) =>
        e.id === id
          ? {
              ...e,
              ...finalUpdates,
              lastEdited: new Date().toLocaleTimeString('en-US', {
                hour: '2-digit',
                minute: '2-digit',
              }),
            }
  ...
  async saveEntryContentToFile(id, html) {
  ...
  deleteEntry(id) {
    ...
    if (entry) {
      ...
      this.db
        .remove('journal_entries', id)
        .catch((e) => logIfTauri('[JournalService] remove entry failed', e));
      this.fs
        .deleteFile('journal', id)
        .catch((e) => console.error('Failed to delete journal file', e));
    }
  }
  ...
  addColumn(column) {
    ...
    this.columns.update((cols) =>
      [...cols, newColumn].sort((a, b) => a.order - b.order),
    );
  ...
  deleteColumn(id) {
    ...
    this.db
      .remove('journal_columns', id)
      .catch((e) => logIfTauri('[JournalService] remove column failed', e));
  }
  // Search
  searchEntries(query, projectId) {
  ...
  updateProjectStats(projectId) {
    ...
    if (project) {
      const progress = project.targetWordCount
        ? Math.min(
            100,
            Math.round((totalWords / project.targetWordCount) * 100),
          )
        : undefined;
