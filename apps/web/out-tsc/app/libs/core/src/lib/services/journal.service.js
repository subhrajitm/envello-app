...
let JournalService = class JournalService {
    ...
    async loadEntryContent(id) {
        ...
        if (entry.content && entry.content.length > 20)
            return entry.content; // Return cached
        ...
        if (mdContent) {
            ...
            this.entries.update(es => es.map(e => e.id === id ? { ...e, content: html } : e));
            return html;
        }
        return '';
    }
    ...
    setActiveProject(id) {
        this.projects.update(projects => projects.map(p => ({ ...p, active: p.id === id })));
    ...
    updateProject(id, updates) {
        this.projects.update(projects => projects.map(p => p.id === id ? {
            ...p,
            ...updates,
    ...
    updateEntry(id, updates) {
        ...
        this.entries.update(entries => entries.map(e => e.id === id ? {
            ...e,
            ...finalUpdates,
    ...
    updateColumn(id, updates) {
        this.columns.update(cols => cols.map(c => c.id === id ? { ...c, ...updates } : c));
        const c = this.columns().find(x => x.id === id);
        if (c)
            this.persistColumn(c);
    }
    deleteColumn(id) {
        const toMove = this.entries().filter(e => e.column === id);
        this.entries.update(entries => entries.map(e => e.column === id ? { ...e, column: 'IDEAS' } : e));
    ...
    searchEntries(query, projectId) {
        ...
        return entries.filter(entry => entry.title.toLowerCase().includes(lowerQuery) ||
