...
let ResearchService = class ResearchService {
    ...
    updateLibrary(id, updates) {
        this.libraries.update(list => list.map(lib => lib.id === id ? { ...lib, ...updates, lastModified: new Date().toISOString().split('T')[0] } : lib));
        const lib = this.libraries().find(l => l.id === id);
        if (lib)
            this.persistLibrary(lib);
    }
    ...
    updateSource(id, updates) {
        this.sources.update(list => list.map(s => s.id === id ? { ...s, ...updates } : s));
        const s = this.sources().find(x => x.id === id);
        if (s)
            this.persistSource(s);
    }
    deleteSource(id) {
        this.sources.update(list => list.filter(s => s.id !== id));
        this.summaries.update(list => list.map(summary => ({
            ...summary,
    ...
    updateSummary(id, updates) {
        this.summaries.update(list => list.map(s => s.id === id ? { ...s, ...updates, lastModified: new Date().toISOString().split('T')[0] } : s));
        const s = this.summaries().find(x => x.id === id);
        if (s)
            this.persistSummary(s);
    }
    deleteSummary(id) {
