...
let NovelContentService = class NovelContentService {
            ...
            const newChapters = novel.chapters.map(group => ({
                ...
                children: group.children.map(chap => {
                    if (chap.id === chapterId) {
                        return { ...chap, content, wordCount, lastEdited: 'Just Now', status: 'EDITING' };
                    }
                    return chap;
                })
            }));
    ...
    updateNote(noteId, title, body) {
        this.activeNovel.update(novel => {
            if (!novel)
                return null;
            const newNotes = novel.notes.map(note => note.id === noteId ? { ...note, title, body, date: 'Just now' } : note);
    ...
    updateCharacter(characterId, updates) {
        this.activeNovel.update(novel => {
            if (!novel)
                return null;
            const newCharacters = novel.characters.map(char => char.id === characterId ? { ...char, ...updates } : char);
    ...
    updateLocation(locationId, updates) {
        this.activeNovel.update(novel => {
            if (!novel)
                return null;
            const newLocations = novel.locations.map(loc => loc.id === locationId ? { ...loc, ...updates } : loc);
    ...
    updateFrontMatterContent(itemId, content, wordCount) {
        this.activeNovel.update(novel => {
            if (!novel)
                return null;
            const updatedFrontMatter = novel.frontMatter.map(item => item.id === itemId
    ...
    updateFrontMatterTitle(itemId, title) {
        this.activeNovel.update(novel => {
            if (!novel)
                return null;
            const updatedFrontMatter = novel.frontMatter.map(item => item.id === itemId ? { ...item, title } : item);
            return { ...novel, frontMatter: updatedFrontMatter };
        });
        this.schedulePersist();
    }
